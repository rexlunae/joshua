...
fn load_mmap(model: &Path, device: &Device) -> QuantizedModel {
    ...
    let mut cursor = Cursor::new(&bytes[..]);
    QuantizedModel::from_gguf_mmap(content, &mut cursor, device, Some(Arc::new(mmap)), None, 0).unwrap()
}
...
fn opencl_qwen3moe_matches_cpu() {
    run_model("qwen3moe", common::write_tiny_qwen3moe_gguf, &[1, 4, 2, 7, 5], true);
}

#[test]
fn opencl_deepseek2_matches_cpu() {
    run_model("deepseek2", common::write_tiny_deepseek2_gguf, &[1, 4, 2, 7, 5], true);
}

#[test]
fn opencl_llama_matches_cpu() {
    run_model("llama", common::write_tiny_llama_gguf, &[1, 4, 2, 7, 5], true);
}
...
fn opencl_deepseek4_matches_cpu() {
    ...
    // the device.
    run_model("deepseek4", common::write_tiny_deepseek4_gguf, &[1, 4, 2, 7, 5], false);
    run_model("deepseek4-kquant", common::write_tiny_deepseek4_gguf_kquant, &[1, 4, 2, 7, 5], false);
}
