...
pub fn call_quantized_matmul_mv_t(
    ...
    call_quantized_matmul_mv_t_impl(
        device, ep, kernels, dtype, (b, m, n, k), lhs, lhs_offset, rhs, 0, dst_offset, dst,
    )
}

/// `call_quantized_matmul_mv_t` with a byte offset for the weight (`rhs`)
/// buffer.  Joshua's vendored addition: lets one no-copy Metal buffer back a
/// whole memory-mapped GGUF file, with each quantized tensor bound at its own
/// file offset.
pub fn call_quantized_matmul_mv_t_zc(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    dtype: GgmlDType,
    ...
    call_quantized_matmul_mv_t_impl(
        device, ep, kernels, dtype, (b, m, n, k), lhs, lhs_offset, rhs, rhs_offset, dst_offset, dst,
    )
}

#[allow(clippy::too_many_arguments)]
fn call_quantized_matmul_mv_t_impl(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    dtype: GgmlDType,
...
pub fn call_quantized_matmul_mm_t(
    ...
    call_quantized_matmul_mm_t_impl(
        device, ep, kernels, dtype, src0_shape, src0_stride, src0, 0, src1_shape, src1_stride,
        src1, src1_offset, dst_shape, dst_offset, dst,
    )
}

/// `call_quantized_matmul_mm_t` with a byte offset for the weight (`src0`)
/// buffer.  See `call_quantized_matmul_mv_t_zc`.
pub fn call_quantized_matmul_mm_t_zc(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    dtype: GgmlDType,
    ...
    call_quantized_matmul_mm_t_impl(
        device, ep, kernels, dtype, src0_shape, src0_stride, src0, src0_offset, src1_shape,
...
pub fn call_quantized_get_rows(
    ...
    call_quantized_get_rows_impl(
        device, ep, kernels, dtype, hidden_size, row_stride, ids_len, src, 0, ids, ids_offset, dst,
    )
}

/// `call_quantized_get_rows` with a byte offset for the weight (`src`)
/// buffer.  See `call_quantized_matmul_mv_t_zc`.
pub fn call_quantized_get_rows_zc(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    dtype: GgmlDType,
    ...
    call_quantized_get_rows_impl(
        device, ep, kernels, dtype, hidden_size, row_stride, ids_len, src, src_offset, ids,
        ids_offset, dst,
    )
}

#[allow(clippy::too_many_arguments)]
fn call_quantized_get_rows_impl(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    dtype: GgmlDType,
