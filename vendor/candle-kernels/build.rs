...
fn main() -> Result<()> {
    ...
    // The WMMA (tensor-core) MoE kernels need compute capability >= 7.0.
    // They export `moe_gemm_wmma` / `moe_gemm_gguf_prefill`, neither of which
    // candle-core 0.11 calls (the live MoE path is `indexed_moe_forward`,
    // compiled to PTX separately), so on pre-Volta parts they are omitted
    // rather than failing the whole crate on `load_matrix_sync`.
    ...
    if compute_cap >= 70 {
        moe_sources.push("src/moe/moe_wmma.cu");
        moe_sources.push("src/moe/moe_wmma_gguf.cu");
    }
