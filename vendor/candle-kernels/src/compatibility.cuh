...
// CUDA 12.4 and later declare __hmax_nan/__hmin_nan for __half in
// cuda_fp16.hpp on *every* architecture -- that declaration sits under no
// __CUDA_ARCH__ guard there, so it is present even for Maxwell (sm_52).
// Redefining it is a hard error, so keep the fallback only for toolkits
// that predate it.
//
// Gate on __CUDACC_VER_* rather than CUDA_VERSION: the latter is defined in
// cuda.h, which these translation units never include, so it would silently
// read as 0 here.
#if (__CUDA_ARCH__ < 800) && ((__CUDACC_VER_MAJOR__ < 12) || \
